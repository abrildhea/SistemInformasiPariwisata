<?php
	...
	$viewWisataHotel = $conn -> prepare("SELECT
	wisata.id_wisata,
...
WHERE
	tipe_wisata.id_tipe_wisata = '4' 
ORDER BY
	wisata.id_wisata ASC");

	$viewWisataHotel -> execute();

	$result = $viewWisataHotel -> fetchAll(PDO::FETCH_ASSOC);
	if (is_null($viewWisataHotel)) {
		$status = false;
	} else {
		$status = true;
	}
	...
	$response = ['status' => $status, 'hotel' => $result];
